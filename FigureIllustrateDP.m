...
gamma_val = 0.6;
target_tube = Tube(gamma_val^0 * safe_set,...
...
[prob_x, cell_xvec, grid_x, mat_prob_x] = SReachDynProg('term', sys, x_inc, ...
    u_inc, target_tube);
...
for itt = 1:length(target_tube)
    ...
    stem3(grid_x, (itt-1)*ones(n_grid_x,1), mat_prob_x(itt,:)','b','LineWidth', 0.8);
...
set(gca,'xtick',-xmax:50*x_inc:xmax);
...
if savefigures
    savefig(gcf,'./matfigs/CartoonForDP.fig','compact');
    saveas(gcf,'./matfigs/CartoonForDP.png');
end
